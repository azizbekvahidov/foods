		...
		if(isset($_POST['Employee']))
		{
			...
			try{
				...
				if($model->save()){
					...
					/*
					$model2 = Employee::model()->findByPk($model->employee_id);						
					if(!empty($uploadFile)) {
						$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
						if(!empty($uploadFile)) {
							if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'employee'.DIRECTORY_SEPARATOR.$model2->employee_id.DIRECTORY_SEPARATOR.$model2->employee_id.'.'.$extUploadFile)){
								$model2->filename=$model2->employee_id.'.'.$extUploadFile;
								$model2->save();
		...
		if(isset($_POST['Employee']))
		{
			...
			try{
				...
				if(!empty($uploadFile)) {
					$extUploadFile = substr($uploadFile, strrpos($uploadFile, '.')+1);
					if(!empty($uploadFile)) {
						if($uploadFile->saveAs(Yii::app()->basePath.DIRECTORY_SEPARATOR.'files'.DIRECTORY_SEPARATOR.'employee'.DIRECTORY_SEPARATOR.$model->employee_id.DIRECTORY_SEPARATOR.$model->employee_id.'.'.$extUploadFile)){
							$model->filename=$model->employee_id.'.'.$extUploadFile;
							$message .= 'and file uploded';
						}
						else{
							$messageType = 'warning';
