...
$this->menu=array(
	array('label'=>'Сотрудники','url'=>array('index'),'icon'=>'fa fa-list-alt', 'items' => $menu)
);
...
<table class="items table table-striped table-hover dataTable table-bordered" id="dataTable">
    ...
    <? $count = 1; foreach($models as $val){?>
        <tr>
            ...
            <td><?=$val->login?></td>
            <td>
                <?=CHtml::link('<i class="icon-eye-open"></i>',$val->employee_id.'|'.$val->name,array('class'=>'view'))?>
                <?=CHtml::link('<i class="icon-pencil"></i>',array('update?id='.$val->employee_id),array('class'=>'update'))?>
                <?=CHtml::link('<i class="icon-trash"></i>',array('delete?id='.$val->employee_id),array('class'=>'delete'))?>
            </td>
        </tr>
        <? $count++;}?>
...
<script>
    ...
    jQuery(document).on('click','#dataTable a.view',function(){
        ...
        $("#myModalBody").load("/employee/view?id="+data[0]+"&asModal=true");
        $("#myModal").modal();
        return false;
    });
