...
$('.search-form form').submit(function(){
	$('#halfstaff-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php $box = $this->beginWidget(
    ...
    array(
        'title' => 'Администрирование полуфабрикатов',
...
<table class="items table table-striped table-hover dataTable table-bordered" id="dataTable">
    ...
    <? $count = 1; foreach($newModel as $val){?>
        <tr>
            <td><?=$count?></td>
            <td><?=$val->halfstuff_id?></td>
            <td><?=$val->name?></td>
            <td>
                <?=CHtml::link('<i class="icon-eye-open"></i>',$val->halfstuff_id.'|'.$val->name,array('class'=>'view'))?>
                <?=CHtml::link('<i class="fa fa-files-o fa-fw"></i>',array('copy?id='.$val->halfstuff_id),array('class'=>'copy'))?>
                <?=CHtml::link('<i class="icon-pencil"></i>',array('update?id='.$val->halfstuff_id),array('class'=>'update'))?>
                <?=CHtml::link('<i class="icon-trash"></i>',array('delete?id='.$val->halfstuff_id),array('class'=>'delete'))?>
            </td>
        </tr>
    <? $count++;}?>
