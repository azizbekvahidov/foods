...
$this->menu=array(
	array('label'=>'Кабинки','url'=>array('index'),'icon'=>'fa fa-list-alt', 'items' => $menu)
);
...
<?php $box = $this->beginWidget(
    ...
    array(
        'title' => 'Администрирование Кабинок',
...
<table class="items table table-striped table-hover" id="dataTable">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?=$value->price?></td>
            <td>
                <?=CHtml::link('<i class="icon-pencil"></i>',array('tables/update?id='.$value->table_num),array('class'=>'update'))?>
                <?=CHtml::link('<i class="icon-trash"></i>',array('tables/delete?id='.$value->table_num),array('class'=>'delete'))?>
            </td>
        </tr>
        <? $count++; }?>
    </tbody>
